...
//=============================================================================
/*:
 * @plugindesc [ ver.1.0.0 ] 對話框視窗擴張按鈕
 * @author あおいたく( 翻譯 : ReIris )
 * @help 該插件將按鈕添加到對話框視窗以執行任意功能。
 *
 * 關於命令參數
 *
 * 當命令是場景時：
 * 通過將 Scene_XXX 的 XXX 部分（Save，Load，Menu，Title 等）指定為參數，
 * 可以在按下按鈕時轉換到相應畫面。
 *
 * 當命令是插件時：
 * 通過在參數中指定插件命令名稱和參數，可以在按下按鈕時執行相應的插件命令。
 *
 * @param ボタン
 * @text 按鈕
 * @desc 設定要在對話框視窗中顯示的按鈕。
 * @type struct<button>[]
 * @default ["{\"text\":\"保存\",\"command\":\"scene\",\"parameters\":\"save\"}","{\"text\":\"讀取\",\"command\":\"scene\",\"parameters\":\"load\"}"]
 */
/*~struct~button:
 * @param text
 * @text 名稱
